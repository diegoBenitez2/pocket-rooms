...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    SET_ROOM(state, { newRoom, rooId }) {
      Vue.set(state.rooms, rooId, newRoom);
    },
    APPEND_ROOM_TO_USER(state, { roomId, userId }) {
      Vue.set(state.user[userId].rooms, roomId, roomId);
    },
  ...
  actions: {
    ...
    FECTH_USERS: ({ state, commit }, { id }) => new Promise((resolve) => {
    ...
    SIGH_IN: (context, { email, password }) => firebase.auth()
